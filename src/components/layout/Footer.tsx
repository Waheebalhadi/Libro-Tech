...
import { Mail, Phone, MapPin, Facebook, Twitter, Linkedin, Instagram, Youtube, ArrowUp, Heart } from 'lucide-react';
...
export default function Footer() {
  ...
  const quickLinks = [
    ...
    { path: '/solutions', label: isRTL ? 'الحلول' : 'Solutions' },
    { path: '/industries', label: isRTL ? 'القطاعات' : 'Industries' },
    { path: '/blog', label: isRTL ? 'المدونة' : 'Blog' },
    { path: '/contact', label: isRTL ? 'تواصل معنا' : 'Contact Us' },
  ];

  const solutions = [
    { path: '/solutions#bpm', label: isRTL ? 'إدارة العمليات' : 'BPM' },
    { path: '/solutions#hrms', label: isRTL ? 'الموارد البشرية' : 'HRMS' },
    { path: '/solutions#crm', label: isRTL ? 'علاقات العملاء' : 'CRM' },
    { path: '/solutions#inventory', label: isRTL ? 'إدارة المخزون' : 'Inventory' },
    { path: '/solutions#scheduling', label: isRTL ? 'جدولة المواعيد' : 'Scheduling' },
    { path: '/solutions#invoicing', label: isRTL ? 'الفواتير' : 'Invoicing' },
  ];

  // Build social links from settings
  const socialLinks = [
    settings?.social_facebook ? { icon: Facebook, href: settings.social_facebook, label: 'Facebook' } : null,
    settings?.social_twitter ? { icon: Twitter, href: settings.social_twitter, label: 'Twitter' } : null,
    settings?.social_linkedin ? { icon: Linkedin, href: settings.social_linkedin, label: 'LinkedIn' } : null,
    settings?.social_instagram ? { icon: Instagram, href: settings.social_instagram, label: 'Instagram' } : null,
    settings?.social_youtube ? { icon: Youtube, href: settings.social_youtube, label: 'YouTube' } : null,
  ...
  const companyName = isRTL 
    ? (settings?.company_name_ar || 'ليبرو تك')
    : (settings?.company_name_en || 'Libro Tech');

  const companyDescription = isRTL
    ? (settings?.company_description_ar || 'شريكك الموثوق في رحلة التحول الرقمي. نقدم حلولاً تقنية متكاملة لإدارة أعمالك بكفاءة واحترافية.')
    : (settings?.company_description_en || 'Your trusted partner in the digital transformation journey. We provide integrated technology solutions to manage your business efficiently and professionally.');

  const companyAddress = isRTL
    ? (settings?.company_address_ar || 'الرياض، المملكة العربية السعودية')
    : (settings?.company_address_en || 'Riyadh, Saudi Arabia');
  ...
  return (
    <footer className="relative bg-gradient-to-b from-foreground to-foreground/95 text-background overflow-hidden" dir={isRTL ? 'rtl' : 'ltr'}>
      {/* Decorative Elements */}
            ...
            <ul className="space-y-3">
              {quickLinks.map((link) => (
                <li key={link.path}>
                  <Link
                    to={link.path}
                    className="text-background/70 hover:text-primary text-sm transition-all duration-300 hover:translate-x-1 inline-block"
                  >
                    {link.label}
                  </Link>
                </li>
        ...
        <div className="border-t border-background/10 pt-8">
          <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
            <p className="text-background/50 text-sm text-center sm:text-start flex items-center gap-1">
              ...
              <Heart className="h-4 w-4 text-primary inline mx-1" />
            </p>
            <div className="flex items-center gap-6 text-sm text-background/50">
              ...
              <button
                onClick={scrollToTop}
                className="w-10 h-10 rounded-xl bg-primary/20 flex items-center justify-center hover:bg-primary hover:scale-110 transition-all duration-300"
